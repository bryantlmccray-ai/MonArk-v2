...
export const AuthPage: React.FC<{ defaultMode?: 'login' | 'signup' }> = ({ defaultMode = 'login' }) => {
    ...
    React.useEffect(() => {
          ...
          toast({
                  ...
                  description: "Welcome to MonArk. Let's set up your profile.",
          });
          // New users always go to onboarding — OnboardingGuard will redirect
                        // back to /dashboard only after is_profile_complete = true
    ...
    const handlePasswordReset = async () => {
          ...
          if (!result.success) {
                  toast({ title: "Invalid email", description: getFirstError(result) || "Please enter a valid email address", variant: "destructive" });
                  return;
          }
          setLoading(true);
          try {
    ...
    const handleGoogleSignIn = async () => {
          ...
          try {
                  ...
                  if (error) {
                            toast({ title: "Google Sign-in failed", description: "Something went wrong with Google sign-in. Please try again.", variant: "destructive" });
                  }
          } catch (error: any) {
                  toast({ title: "Error", description: "Something went wrong with Google sign-in. Please try again.", variant: "destructive" });
          } finally {
                  setLoading(false);
          }
    };
    ...
    const handleAgeVerification = async (ageData: { dateOfBirth: Date; ageConfirmed: boolean }) => {
          ...
          try {
                  const signUpOptions: Parameters<typeof supabase.auth.signUp>[0] = {
                            ...
                            options: {
                                        // After email confirmation, send users to onboarding
            ...
            if (data.session && data.user) {
                      ...
                      // useEffect on `user` will navigate to /onboarding
                    return;
            }
    ...
    if (pendingVerificationEmail) {
          return (
                  <div className="min-h-screen flex items-center justify-center px-6 relative overflow-hidden bg-background">
                          <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full pointer-events-none opacity-40"
                                      style={{ background: "radial-gradient(circle, hsl(var(--primary) / 0.08) 0%, transparent 70%)" }} />
                          <motion.div className="w-full max-w-sm space-y-6 relative z-10"
                                    ...
                                    <div className="bg-card border border-border/60 rounded-2xl p-6 space-y-5 shadow-[var(--shadow-elevated)] text-center">
                                                <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto bg-primary/10 text-primary text-xl font-semibold">✓</div>div>
                                                <div className="space-y-2">
                                                              <h1 className="text-2xl font-serif font-bold tracking-tight text-foreground">Check your email</h1>h1>
                                                              <p className="font-body text-sm leading-relaxed text-muted-foreground">
                                                                              We sent a verification link to <span className="font-medium text-foreground">{pendingVerificationEmail}</span>span>.
                                                              </p>p>
                                                              <p className="font-body text-sm leading-relaxed text-muted-foreground">
                                                                              Once you confirm your email, you'll be taken directly to set up your profile.
                                                              </p>p>
                                                </div>div>
                                                <div className="space-y-3 pt-1">
                                                        ...
                                                        setEmail(email || pendingVerificationEmail);
                                                              ...
                                                              <Button variant="ghost" className="w-full rounded-xl text-muted-foreground" onClick={() => {
                                                        setPendingVerificationEmail(null);
                                                        setIsLogin(false);
                                                        setEmail('');
                                                        setPassword('');
                ...
                <motion.div className="w-full max-w-sm space-y-6 relative z-10"
                        ...
                        <div className="text-center space-y-2">
                                  ...
                                  <p className="font-body text-sm text-muted-foreground">
                                    {isLogin ? 'Sign in to continue your journey' : 'Create your account to start meaningful connections'}
                                  </p>p>
                        </div>div>
                        <div className="bg-card border border-border/60 rounded-2xl p-6 space-y-5 shadow-[var(--shadow-elevated)]">
                                  <form onSubmit={handleSubmit} className="space-y-4">
                                    {!isLogin && (
                                          <div className="space-y-1.5">
                                                          <Label htmlFor="name" className="text-sm font-medium text-foreground">Full Name</Label>Label>
                                                          <Input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)}
                                                                              placeholder="Enter your full name" className="rounded-xl h-11" required={!isLogin} maxLength={100} />
                                          </div>div>
                                              )}
                                              <div className="space-y-1.5">
                                                            <Label htmlFor="email" className="text-sm font-medium text-foreground">Email Address</Label>Label>
                                                            <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                                                                              placeholder="Enter your email" className="rounded-xl h-11" required maxLength={255} />
                                              </div>div>
                                              <div className="space-y-1.5">
                                                            <Label htmlFor="password" className="text-sm font-medium text-foreground">Password</Label>Label>
                                                            <div className="relative">
                                                                            <Input id="password" type={showPassword ? 'text' : 'password'} value={password}
                                                                                                onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password"
                                                                                                className="rounded-xl h-11 pr-10" required maxLength={128} />
                                                                            <button type="button" onClick={() => setShowPassword(!showPassword)}
                                                                                                className="absolute right-3 top-1/2 -translate-y-1/2 text-primary/60 hover:text-primary transition-colors" tabIndex={-1}>
                                                                              {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                                                                            </button>button>
                                                            </div>div>
                                                {!isLogin && <p className="text-xs text-muted-foreground">Must be at least 6 characters</p>p>}
                                              </div>div>
                                    {isLogin && (
                                          <div className="flex items-center gap-3">
                                                          <Checkbox id="remember-me" checked={rememberMe} onCheckedChange={(checked) => setRememberMe(checked as boolean)}
                                                                              className="border-primary/40 data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
                                                          <Label htmlFor="remember-me" className="text-sm text-muted-foreground cursor-pointer">Remember me</Label>Label>
                                          </div>div>
                                              )}
                                    {!isLogin && (
                                          <>
                                                          <div className="flex items-start gap-3 pt-1">
                                                                            ...
                                                                            <p className="text-sm leading-relaxed text-muted-foreground">
                                                                                                I agree to MonArk's{' '}
                                                                                                <a href="/terms" className="text-primary font-medium hover:underline" target="_blank" rel="noopener noreferrer">Terms of Service</a>a>{' '}
                                                                                                and{' '}
                                                                                                <a href="/privacy" className="text-primary font-medium hover:underline" target="_blank" rel="noopener noreferrer">Privacy Policy</a>a>.
                                                                            </p>p>
                                                          </div>div>
                                            {TURNSTILE_SITE_KEY && (
                                              ...
                                              <motion.button type="submit" disabled={loading || (!isLogin && !agreedToTerms)}
                                                ...
                                                {loading ? (
                                                                                <div className="flex items-center justify-center space-x-2">
                                                                                                  <div className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin"></div>div>
                                                                                                  <span>{isLogin ? 'Signing in...' : 'Creating account...'}</span>span>
                                                                                </div>div>
                                                                              ) : (isLogin ? 'Sign In' : 'Create Account')}
                                              </motion.button>motion.button>
                                  </form>form>
                                  <div className="relative">
                                              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-border"></div>div></div>div>
                                              <div className="relative flex justify-center text-xs"><span className="px-3 bg-card text-muted-foreground">or</span>span></div>div>
                                  </div>div>
                                  ...
                                  <p className="text-xs text-muted-foreground text-center pt-1 font-medium">🔒 Your data is encrypted and secure</p>p>
                        </div>div>
                        <div className="text-center space-y-1.5">
                                  <button onClick={() => { setIsLogin(!isLogin); setShowForgotPassword(false); setResetSent(false); }}
                                                className="transition-colors font-body text-sm text-primary hover:text-primary/80">
                                    {isLogin ? "Don't have an account? Sign up" : "Already have an account? Sign in"}
                                  </button>button>
                          {isLogin && (
                                        <div>
                                                      <button onClick={() => { setShowForgotPassword(!showForgotPassword); setResetEmail(email); }}
                  ...
                  {showForgotPassword && (
                                      <div className="bg-card border border-border/60 rounded-2xl p-6 space-y-4 shadow-[var(--shadow-elevated)]">
                                                  <div className="text-center">
                                                                <h3 className="font-serif font-semibold text-foreground mb-1">Reset Password</h3>h3>
                                                                <p className="text-sm font-body text-muted-foreground">Enter your email and we'll send you a reset link.</p>p>
                                                  </div>div>
                                        {!resetSent ? (
                                                      <div className="space-y-4">
                                                                      <div className="space-y-1.5">
                                                                                        <Label htmlFor="reset-email" className="text-sm font-medium text-foreground">Email Address</Label>Label>
                                                                                        <Input id="reset-email" type="email" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)}
                                                                                                              placeholder="Enter your email" className="rounded-xl h-11" maxLength={255} />
                                                                      </div>div>
                                                      ...
                                                      <div className="text-center py-4">
                                                                      <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 bg-primary/10">
                                                                                        <span className="text-xl text-primary">✓</span>span>
                                                                      </div>div>
                                                                      <p className="mb-1 font-body font-medium text-foreground">Check your email!</p>p>
                                                                      <p className="text-sm font-body text-muted-foreground">
                                                                                        Reset link sent to <span className="text-primary font-medium">{resetEmail}</span>span>
          </p>p>
                                 </div>div>
                                                  )}
                    </div>
