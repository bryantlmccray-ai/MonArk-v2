...
import { RIFResultsScreen } from './RIFResultsScreen';
...
export const RIFQuiz: React.FC<RIFQuizProps> = ({ onComplete, onBack }) => {
    ...
    // Show results screen after completing all questions
    if (showResults) {
          return (
                  <RIFResultsScreen
                            answers={answers}
                            onViewProfile={() => onComplete(answers)}
                            onRetake={() => {
    ...
    const handleNext = () => {
          if (currentQuestion < RIF_QUESTIONS.length - 1) {
                  ...
                  // Last question — show results screen instead of immediately completing
            setShowResults(true);
          }
    };
            ...
            {/* Footer */}
                <div className="px-6 pb-8 pt-4 max-w-2xl mx-auto w-full">
                        <Button
                                  ...
                                  >
                          {currentQuestion === RIF_QUESTIONS.length - 1 ? 'See My Results' : 'Continue'}
                        </Button>Button>
                </div>div>
          </div>div>
        );
