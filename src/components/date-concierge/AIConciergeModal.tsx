...
export const AIConciergeModal: React.FC<AIConciergeModalProps> = ({
    ...
    // Fixed: added proposed_time to the detail payload so the saved plan uses the
    // AI-suggested time rather than always defaulting to right now.
    const handleSaveToPlan = () => {
          if (!generatedProposal) return;
          const topVenue = venueRecommendations[0] ?? null;

          // Resolve a concrete proposed_time from the time_suggestion string when possible.
          // We default to the next occurrence of the suggested period (e.g. "Saturday morning").
          // If we can't parse it we pass undefined and let DatePlansTab fall back to now.
          let proposedTime: string | undefined;
          try {
                  const suggestion = (generatedProposal.time_suggestion ?? '').toLowerCase();
                  const target = new Date();
                  // Advance to next Saturday if suggestion mentions weekend
            if (suggestion.includes('saturday')) {
                      const day = target.getDay();
                      target.setDate(target.getDate() + ((6 - day + 7) % 7 || 7));
            } else if (suggestion.includes('sunday')) {
                      const day = target.getDay();
                      target.setDate(target.getDate() + ((0 - day + 7) % 7 || 7));
            } else {
                      // Default: next available day at a reasonable hour
                    target.setDate(target.getDate() + 1);
            }
                  // Set hour based on morning/afternoon/evening hint
            if (suggestion.includes('morning')) target.setHours(10, 0, 0, 0);
                  else if (suggestion.includes('afternoon')) target.setHours(14, 0, 0, 0);
                  else if (suggestion.includes('evening') || suggestion.includes('night')) target.setHours(19, 0, 0, 0);
                  else target.setHours(12, 0, 0, 0);
                  proposedTime = target.toISOString();
          } catch {
          ...
          // Navigate user to the plans tab so they see the result immediately
          window.dispatchEvent(new CustomEvent('monark-navigate', { detail: { tab: 'date-plans' } }));
          setSavedToPlan(true);
    };
          ...
          <div className="space-y-6">
                ...
                <div className="bg-muted/50 rounded-xl p-6 space-y-4 border border-border">
                        <div className="flex items-center space-x-4 text-sm text-foreground/80">
                                  ...
                                  <span className="font-medium">Analyzing your conversation flow</span>span>
                        </div>div>
                        <div className="flex items-center space-x-4 text-sm text-foreground/80">
                                  <Heart className="h-5 w-5 text-destructive/60" />
                                  <span className="font-medium">Matching shared interests and values</span>span>
                        </div>div>
                        <div className="flex items-center space-x-4 text-sm text-foreground/80">
                                  <MapPin className="h-5 w-5 text-primary" />
                                  <span className="font-medium">Curating location and activity options</span>span>
                        </div>div>
                </div>div>
                <div className="flex space-x-4 pt-2">
                        <Button onClick={onClose} variant="outline" className="flex-1">Maybe Later</Button>Button>
                        <Button onClick={handleGenerateProposal} className="flex-1">Let's Plan This!</Button>Button>
                </div>div>
          </div>div>
        );
                  ...
                  <div className="space-y-6">
                          ...
                          <div className="bg-muted/50 rounded-2xl p-8 border border-border">
                                    ...
                                    <div className="space-y-4 mb-6">
                                                <div className="flex items-center space-x-4 text-sm">
                                                              <Heart className="h-5 w-5 text-destructive/60" />
                                                              <span className="text-foreground/80 font-medium">Vibe: {generatedProposal.vibe}</span>span>
                                                </div>div>
                                                <div className="flex items-center space-x-4 text-sm">
                                                              <MapPin className="h-5 w-5 text-primary" />
                                                              <span className="text-foreground/80 font-medium">Setting: {generatedProposal.location_type}</span>span>
                                                </div>div>
                                                <div className="flex items-center space-x-4 text-sm">
                                                              <Clock className="h-5 w-5 text-accent" />
                                                              <span className="text-foreground/80 font-medium">Timing: {generatedProposal.time_suggestion}</span>span>
                                                </div>div>
                                    </div>div>
                                    ...
                                    <div className="bg-secondary rounded-xl p-4 border border-border">
                                                <p className="text-foreground text-sm font-semibold mb-2 tracking-wide">Activity Details:</p>p>
                                                <p className="text-foreground/70 text-sm font-medium leading-relaxed">{generatedProposal.activity}</p>p>
                                    </div>div>
                          </div>div>
                    ...
                    {/* Save to Plans action */}
                          <button
                                    ...
                                    >
                            {savedToPlan ? (
                                                  <><Check className="h-4 w-4" /> Saved to My Plans</>>
                                                ) : (
