...
import { User, MessageCircle, BookOpen, Calendar, Compass, Brain, Share2, Navigation, CalendarCheck } from 'lucide-react';
...
function useNavBadges() {
  ...
  // Badge: count of proposed (unconfirmed) itineraries so users know plans need attention
  const { data: pendingPlanCount = 0 } = useQuery({
        queryKey: ['nav-pending-plans', user?.id],
        queryFn: async () => {
                if (!user?.id) return 0;
                const { count } = await supabase
                  .from('itineraries')
  ...
  const rifNotComplete = !rifProfile;
    return { unreadCount, newMatchCount, rifNotComplete, pendingPlanCount };
}
...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({ activeTab, onTabChange }) => {
    const { unreadCount, newMatchCount, rifNotComplete, pendingPlanCount } = useNavBadges();
  
    // 9 tabs — added 'date-plans' using the already-imported CalendarCheck icon
    const tabs = [
      { id: 'weekly',     icon: Calendar,      label: 'Your 3',    badge: newMatchCount },
      { id: 'discover',  icon: Navigation,    label: 'Discover',  badge: 0 },
      { id: 'matches',   icon: MessageCircle, label: 'Connect',   badge: unreadCount },
      { id: 'dates',     icon: BookOpen,      label: 'Journal',   badge: 0 },
      { id: 'compass',   icon: Compass,       label: 'Compass',   badge: 0 },
      { id: 'date-plans', icon: CalendarCheck, label: 'My Plans', badge: pendingPlanCount },
      { id: 'rif',       icon: Brain,         label: 'Profile',   badge: rifNotComplete ? 1 : 0 },
                  ...
                  {tabs.map((tab) => {
                            ...
                            return (
                                          <button
                                                          ...
                                                          className={`flex flex-col items-center justify-center py-1.5 px-1 min-w-0 flex-1 rounded-xl transition-all duration-200 ease-out active:scale-[0.92] ${
                                                                            isActive
                                                                              ? 'text-primary'
                                                        ...
                                                        <div className="relative mb-0.5">
                                                          {isActive && (
                                                                        ...
                                                                        <Icon
                                                                                            className={`h-[18px] w-[18px] relative transition-all duration-200 ${
