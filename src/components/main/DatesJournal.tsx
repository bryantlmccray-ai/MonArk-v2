    ...
    <div className="space-y-4">
      ...
      {earned.length === 0 && upcoming.length === 0 && (
        <div className="text-center py-10 bg-card rounded-2xl border border-border/60">
          ...
          <p className="font-serif italic text-foreground text-base mb-2">Your milestones live here.</p>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex gap-1 bg-muted rounded-xl p-1 w-fit mx-auto">
          {(['journal', 'milestones'] as const).map((t) => (
            ...
            >
              {t === 'journal' ? (
                <span className="flex items-center gap-1.5"><BookOpen className="w-3 h-3" /> Journal</span>
              ) : (
                <span className="flex items-center gap-1.5"><Trophy className="w-3 h-3" /> Milestones</span>
              )}
            </button>
          ))}
        </div>
