  ...
  return (
    ...
    >
      ...
      {itinerary.status === 'confirmed' && (
        <div className="flex gap-2">
          <Button size="sm" className="flex-1 h-8 text-xs" onClick={() => onStatusChange(itinerary.id, 'completed')}>Mark Completed</Button>
          {mapsUrl && (
            <a
              href={mapsUrl}
              target="_blank"
...
export const DatePlansTab: React.FC<DatePlansTabProps> = ({ onNavigateCompass }) => {
  ...
  React.useEffect(() => {
    const handler = async (e: Event) => {
      ...
      const now = new Date();
      const proposedStart = detail.proposed_time ? new Date(detail.proposed_time) : now;
