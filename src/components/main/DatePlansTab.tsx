...
const ItineraryCard: React.FC<{
    ...
    // Cross-reference venues list by venue_id to enrich card with address and Maps link
    const venue = venues.find((v) => v.id === itinerary.location_data?.venue_id) ?? null;
    ...
    return (
          <motion.div
                ...
                <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs text-muted-foreground">{formattedDate}</span>span>
                </div>div>
                <h3 className="font-serif text-base font-semibold text-foreground leading-snug mb-1">{itinerary.title}</h3>h3>
            {itinerary.description && (
                          <p className="text-xs text-muted-foreground leading-relaxed mb-3 line-clamp-2">{itinerary.description}</p>p>
                )}
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mb-4">
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{formattedTime}</span>span>
                  {locationName && (
                            mapsUrl ? (
                                          <a
                                                          href={mapsUrl}
                                          ...
                                          <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{locationName}</span>span>
                                        )
                          )}
                </div>div>
            {itinerary.status === 'proposed' && (
                          <div className="flex gap-2">
                                    <Button size="sm" variant="outline" className="flex-1 h-8 text-xs border-primary/30 text-primary hover:bg-primary/5" onClick={() => onStatusChange(itinerary.id, 'confirmed')}>Confirm</Button>Button>
                                    <Button size="sm" variant="ghost" className="flex-1 h-8 text-xs text-muted-foreground hover:text-destructive" onClick={() => onStatusChange(itinerary.id, 'cancelled')}>Cancel</Button>Button>
                          </div>div>
                )}
            {itinerary.status === 'confirmed' && (
                          <div className="flex gap-2">
                                    <Button size="sm" className="flex-1 h-8 text-xs" onClick={() => onStatusChange(itinerary.id, 'completed')}>Mark Completed</Button>Button>
                            {mapsUrl && (
                                        <a
                                                        href={mapsUrl}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="flex items-center gap-1 px-3 h-8 text-xs rounded-md border border-border text-muted-foreground hover:text-primary hover:border-primary/30 transition-colors"
                                                      >
...
const EmptyState: React.FC<{ onNavigateCompass: () => void }> = ({ onNavigateCompass }) => (
    <div className="text-center py-16 bg-card rounded-2xl border border-border/60">
        ...
        <p className="font-serif italic text-foreground text-xl mb-2">No date plans yet.</p>p>
                  ...
                  {open && (
                      <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute bottom-full left-0 mb-2 w-60 bg-card border border-border rounded-xl p-3 z-50 shadow-lg">
                                  <p className="text-xs text-muted-foreground leading-relaxed">
                                                Your <span className="font-semibold text-foreground">{pillarLabel}</span>span> RIF score is{' '}
                                                <span className="font-semibold text-primary">{pillarScore}</span>span>. This rhythm was pre-selected because it aligns with where you are right now. Switch freely to explore other energy modes.
                                  </p>p>
                      </motion.div>motion.div>
                    )}
...
export const DatePlansTab: React.FC<DatePlansTabProps> = ({ onNavigateCompass }) => {
    ...
    const rifScores = {
          ...
          // Fixed: was incorrectly using rifProfile.boundary_respect — must be growth_orientation
          growth_orientation: rifProfile ? normalizeRIFScore(rifProfile.growth_orientation) : 50,
    };
    ...
    const filterByRhythm = (items: Itinerary[]): Itinerary[] => {
          ...
          return items.filter((it) => {
                  ...
                  // If plan has an explicit rhythm tag, match it exactly
                  if (tag) return tag === rhythmMode;
                  // Fixed: plans with no rhythm tag are shown for 'spark' (default pass-through)
                  // and filtered by title keywords only for 'reset' and 'stretch' to avoid
                  // silently hiding plans that were created outside the Compass flow
                  ...
                  // 'spark' — show all untagged plans so nothing is lost
                  return true;
          });
    };
    ...
    React.useEffect(() => {
          const handler = async (e: Event) => {
                  ...
                  // Fixed: use proposed_time from the event detail when available so plans
                  // don't all default to "right now" — falls back to current time if absent
                  const proposedStart = detail.proposed_time
                            ? new Date(detail.proposed_time)
                            : now;
                  const proposedEnd = new Date(proposedStart.getTime() + 3 * 60 * 60 * 1000);
                  ...
                  const { error } = await supabase.from('itineraries').insert({
                            ...
                            time_window: { start: proposedStart.toISOString(), end: proposedEnd.toISOString() },
                  ...
                  if (error) { toast.error('Could not save plan'); } else { toast.success('Plan saved to My Plans!'); refetch(); }
          };
          ...
          <div className="min-h-full bg-background">
                <div className="px-5 pt-6 pb-4 border-b border-border/40">
                        <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <span className="text-[10px] font-caption text-primary tracking-widest uppercase">Smart Date Planner</span>span>
                        </div>div>
                        <h1 className="font-serif text-2xl font-semibold text-foreground leading-tight">My Date Plans</h1>h1>
                        <p className="text-sm text-muted-foreground mt-1 leading-snug">Plans saved from Compass — confirmed or waiting, all in one place.</p>p>
                ...
                <div className="px-5 pt-5 pb-10 space-y-6">
                        <div>
                                  <div className="flex items-center gap-2 mb-2">
                                              <p className="text-[10px] font-caption text-muted-foreground/60 uppercase tracking-widest">Rhythm Filter</p>p>
                          ...
                          return (
                                            <button key={mode.id} onClick={() => setRhythmMode(mode.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium transition-all ${isActive ? 'bg-primary/10 border-primary/30 text-primary' : 'bg-card border-border text-muted-foreground hover:border-primary/20 hover:text-foreground'}`}>
                                              ...
                                              {mode.label}
                                              {isDefault && !isActive && <span className="text-[8px] font-caption text-primary/60 bg-primary/10 rounded-full px-1">RIF</span>span>}
                                            </button>button>
                                          );
          })}
                                  </div>div>
                                  <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">{activeRhythmConfig.description}</p>p>
                        </div>div>
                        ...
                        <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} onClick={onNavigateCompass} className="w-full flex items-center justify-between px-4 py-3.5 rounded-2xl text-left border border-primary/20 bg-primary/5 hover:bg-primary/10 transition-colors">
                                  <div className="flex items-center gap-3">
                                              <div className="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><Compass className="w-4 h-4 text-primary" /></div>div>
                                              <div>
                                                            <p className="text-xs font-semibold text-foreground">Discover via Compass</p>p>
                                                            <p className="text-[11px] text-muted-foreground mt-0.5">Venues matched to your RIF reading → save here instantly</p>p>
                                              </div>div>
                                  </div>div>
                  ...
                  {loading ? (
                      <div className="space-y-3">{[1, 2].map((i) => <div key={i} className="h-28 bg-muted rounded-2xl animate-pulse" />)}</div>div>
...
export default DatePlansTab;</motion.div>
