...
const DIM_CONFIGS: Array<{
  ...
  { pctKey: 'intent_clarity_pct', label: 'Intent Clarity', description: "How clearly you know what you are looking for.", barColor: 'bg-primary', textColor: 'text-primary' },
  { pctKey: 'emotional_readiness_pct', label: 'Emotional Readiness', description: 'Your capacity to show up present and open.', barColor: 'bg-accent', textColor: 'text-accent' },
  { pctKey: 'pacing_preferences_pct', label: 'Pacing Preference', description: 'The rhythm that feels natural to you.', barColor: 'bg-[hsl(350,30%,62%)]', textColor: 'text-[hsl(350,30%,62%)]' },
  { pctKey: 'boundary_respect_pct', label: 'Boundary Respect', description: 'How you hold and honor limits — yours and others.', barColor: 'bg-[hsl(22,38%,36%)]', textColor: 'text-[hsl(22,38%,36%)]' },
  { pctKey: 'post_date_alignment_pct', label: 'Post-Date Alignment', description: 'Your follow-through and communication after connection.', barColor: 'bg-muted-foreground', textColor: 'text-muted-foreground' },
];

const RIFResultsDashboard: React.FC<{ rifProfile: RIFProfileType; onRetake: () => void }> = ({ rifProfile, onRetake }) => {
  const avgPct = (rifProfile.intent_clarity_pct + rifProfile.emotional_readiness_pct + rifProfile.pacing_preferences_pct + rifProfile.boundary_respect_pct + rifProfile.post_date_alignment_pct) / 5;
  const archetype =
    avgPct >= 80 ? { label: 'The Intentional Partner', tagline: 'You date with clarity, depth, and purpose.' }
    : avgPct >= 60 && rifProfile.emotional_readiness_pct >= 70 ? { label: 'The Hopeful Romantic', tagline: "You lead with your heart and that is a strength." }
    : { label: 'The Guarded Opener', tagline: "You are learning to open on your own terms." };

  return (
    <div className="space-y-5 px-5 pt-3 pb-8">
      <div>
        <p className="text-[11px] tracking-[0.2em] text-primary uppercase font-medium mb-1">Relational Intelligence Framework</p>
        <h1 className="text-2xl font-serif font-bold text-foreground">Your RIF Profile</h1>
        <p className="text-sm text-muted-foreground mt-0.5">Your dating intelligence snapshot — updated as you engage with MonArk</p>
      </div>
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-xl px-4 py-3 flex items-start gap-2.5">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground leading-relaxed mt-0.5">Answer daily questions on the Home tab — each response sharpens your Sunday matches without requiring another sit-down quiz. Your scores update automatically.</p>
        </div>
      </div>
      <div className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
        <p className="text-[11px] tracking-[0.15em] text-primary uppercase font-medium mb-4">Your Dimensions</p>
            ...
            return (
              <div key={pctKey as string}>
                ...
                <div className="h-[3px] bg-secondary rounded overflow-hidden">
                  <motion.div className={`h-full rounded ${barColor}`} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.9, ease: [0.25, 0.46, 0.45, 0.94] }} />
                </div>
              </div>
            );
          })}
      ...
      <button onClick={onRetake} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors mx-auto">
...
export const MainApp: React.FC<MainAppProps> = ({ initialTab = 'weekly' }) => {
  ...
  const { pendingFeedback, showFeedback, loading: feedbackLoading, setShowFeedback, submitFeedback } = useContactFeedback();
  ...
  useEffect(() => { recordSession(); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  useEffect(() => {
    if (activeTab === 'weekly') recordWeeklyOptionsView(); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTab]);
    ...
    switch (activeTab) {
      ...
      case 'rif':
        ...
        return (
          <RIFQuiz
            userId={user?.id || ''}
            onComplete={() => { setRifRetakeMode(false); handleTabChange('profile'); }}
            onSkip={() => { setRifRetakeMode(false); handleTabChange('profile'); }}
          />
        );
      case 'profile':
        return (
  ...
  if (isMobile) {
    return (
      <div className="h-screen bg-background relative flex flex-col overflow-hidden isolate">
        ...
        >
          <img src="/lovable-uploads/e11ccc80-2237-4aac-b579-dccb89f8d727.png" alt="MonArk" className="h-8 w-8 rounded-full object-contain" />
          <div className="flex items-center gap-3">
            {user && <NotificationBell userId={user.id} />}
            <button onClick={() => setShowSettings(true)} disabled={isSigningOut} className="p-2 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-xl transition-all active:scale-95" aria-label="Settings">
              <Settings className="h-5 w-5" />
            </button>
            <button onClick={handleSignOut} disabled={isSigningOut} className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-xl transition-all disabled:opacity-50 active:scale-95" aria-label="Sign out">
              {isSigningOut ? (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button onClick={dismissDailyQ} className="text-xs text-muted-foreground hover:text-foreground">Skip today</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <PostDateReflection isOpen={showPostDateReflection} onClose={() => setShowPostDateReflection(false)} partnerName={reflectionPartnerName} />
        {pendingFeedback && (
          <ContactShareFeedback matchName={pendingFeedback.matchName} open={showFeedback} onClose={() => setShowFeedback(false)} onSubmit={submitFeedback} loading={feedbackLoading} />
        )}
