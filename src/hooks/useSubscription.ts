 ...
 *
 * Loading behaviour:
 *   hasFeature() returns true optimistically while loading so paying users
 *   never see a flash of "feature hidden". Once the real tier is confirmed,
 *   if the user is actually on free, the paywall will show at that point.
 */
...
export function useSubscription() {
  ...
  /**
     ...
     *
     * Optimistic loading: while the subscription state is still being fetched
     * we return `true` for all features so that paying users never see a flash
     * of locked/hidden UI. Once loading resolves, the real tier is enforced.
     * Free-tier users will see a brief optimistic grant that is then corrected —
     * this is an acceptable trade-off because the paywall check happens at the
     * action level (requireFeature) not just at render time.
     */
  const hasFeature = useCallback(
        (featureName: string): boolean => {
                // Optimistic: grant access while loading to avoid flash for paying users
          ...
          const tierActive = state.status === "active" || state.tier === "free";
