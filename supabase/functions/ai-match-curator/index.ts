...
interface CurationInput {
  user_a_id: string
  user_b_id: string
  requester_id: string          // the user for whom we are curating (gets the 3 weekly picks)
}
...
interface CurationResult {
  ...
  match_reason: string          // natural language, shown to user
  internal_rationale: string    // full reasoning, stored for audit
  rif_breakdown: Record<string, number>
  interest_overlap: string[]
  flags: string[]               // e.g. ["pacing_mismatch", "high_rif_alignment"]
}
...
async function curatePair(supabase: any, input: CurationInput): Promise<Response> {
  ...
  const [profileA, profileB, rifA, rifB, weightsA, feedbackA, feedbackB] = await Promise.all([
    ...
    fetchCompatibilityFeedback(supabase, user_b_id),
  ])

  // 2. Build deterministic feature vector
  const features = buildFeatureVector(profileA, profileB, rifA, rifB, feedbackA, weightsA)

  // 3. Ask GPT-4o-mini to make a considered curation decision
  const aiDecision = await callGPTCurator(profileA, profileB, rifA, rifB, features, feedbackA)

  // 4. Merge deterministic scores with AI reasoning
  const result: CurationResult = {
    should_match: aiDecision.should_match,
    confidence: aiDecision.confidence,
    compatibility_score: Math.round(features.weighted_score * 100),
  ...
  // 5. Persist curation decision for audit + future learning
...
async function batchCurate(supabase: any, data: any): Promise<Response> {
  ...
  // Score every candidate individually (could be parallelized further)
  ...
  // Sort by confidence * compatibility_score, return top N
  scored.sort((a, b) =>
    (b.result.confidence * b.result.compatibility_score) -
    (a.result.confidence * a.result.compatibility_score)
  )

  const picks = scored.slice(0, top_n)

  return new Response(JSON.stringify({ success: true, picks, total_evaluated: candidate_ids.length }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' }
  })
}
...
function buildFeatureVector(
  ...
  // RIF dimension compatibility (proximity-based: closer = more compatible)
  ...
  if (rifA && rifB) {
    for (const dim of rifDimensions) {
      ...
      // Weighted proximity: boundary_respect and emotional_readiness matter most
    ...
    // Normalize by total weight (1.5 + 1.3 + 1 + 1 + 1 = 5.8)
    rifScore = rifScore / 5.8
  } else {
    // Partial data: use neutral baseline
    rifScore = 0.5
  ...
  // Behavioral score: learn from B's swipe patterns in feedback history
  // If requester previously liked/messaged people similar to B, boost score
  const behavioralScore = calculateBehavioralScore(profileB, interestsB, feedbackHistory)

  // Apply personalized weights from ML trainer (fall back to defaults if cold start)
  const w = weights ?? { rif_weight: 0.45, interest_weight: 0.30, behavioral_weight: 0.25 }
  const weightedScore =
    rifScore * w.rif_weight +
    interestScore * w.interest_weight +
    behavioralScore * w.behavioral_weight
...
function calculateBehavioralScore(
  ...
  // Find historical targets the requester positively engaged with
  ...
  // Score = how many of candidate's interests appear in interests of liked people
  // (We'd need those profiles too for full clustering — approximate with score distribution)
  ...
  // Simple approximation: normalize avg positive score to 0-1
...
async function callGPTCurator(
  ...
  if (!openAIKey) {
    // Fallback to deterministic decision when no key
    return deterministicFallback(features)
  }
...
You will be given:
- Two user profiles with interests and bio context
- Their RIF (Relational Intelligence Framework) scores (1-10) across 5 dimensions
- A compatibility feature vector already computed
- A summary of the requester's past engagement patterns
...
4. Write internal_rationale (3-5 sentences, honest analysis for audit, can mention concerns)
5. List flags from: high_rif_alignment, pacing_mismatch, boundary_concern, high_interest_overlap, low_data_confidence, behavioral_positive_signal, emotional_readiness_gap

Rules:
- match_reason must NEVER be generic. It must reference actual interests or RIF qualities.
...
- Write match_reason in second person to Person A (the requester). E.g. "You both light up around..."
...
weighted_score: ${Math.round(features.weighted_score * 100)}%

Person A's engagement history summary: ${recentFeedbackSummary}
  ...
  try {
    const resp = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.4,   // low temp = consistent, considered decisions
        max_tokens: 400,
    ...
    if (!content) return deterministicFallback(features)
...
function deterministicFallback(features: ReturnType<typeof buildFeatureVector>) {
  ...
  return {
    ...
    internal_rationale: `Deterministic fallback (no OpenAI key). Weighted score: ${Math.round(score * 100)}%. RIF: ${Math.round(features.rif_score * 100)}%, Interests: ${Math.round(features.interest_score * 100)}%, Behavioral: ${Math.round(features.behavioral_score * 100)}%.`,
    flags: score > 0.75 ? ['high_rif_alignment'] : score < 0.45 ? ['low_data_confidence'] : [],
  }
}
...
async function fetchMLWeights(supabase: any, userId: string): Promise<MLWeights> {
  const { data } = await supabase
    ...
    .eq('user_id', userId)
    .maybeSingle()
  // Fall back to default weights on cold start
